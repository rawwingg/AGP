...
#DATA_PATH = Path("Polygons") / "polygons.json" not using this path anymore, using local_dataset_with_guards.json instead
LOCAL_DATASET_PATH = Path("Polygons") / "local_dataset_with_guards.json"
CANVAS_PADDING = 40


class GuardedPolygonViewer(tk.Tk):
    def __init__(self, data_path=LOCAL_DATASET_PATH):
        super().__init__()
        self.title("GuardedPolygon Dataset Viewer")
    ...
    def draw_selected_polygon(self):
        ...
        self.meta_label.configure(
            text=f"{len(outer_points)} outer points  |  {len(holes)} holes  |  area {record['area']:.2f}  |  guards {len(guards)}  |  coverage {record['area_coverage'] * 100:.2f}%"
        )
        ...
        for guard in guards:
            ...
            # get exterior coords (if MultiPolygon, use largest polygon)
            if coverage_polygon.geom_type == "Polygon":
                cov_coords = list(coverage_polygon.exterior.coords)
            elif coverage_polygon.geom_type.startswith("Multi") or coverage_polygon.geom_type == "GeometryCollection":
                polys = [g for g in coverage_polygon.geoms if g.geom_type == "Polygon"]
                if not polys:
                    continue
                # choose largest by area
                main = max(polys, key=lambda p: p.area)
                cov_coords = list(main.exterior.coords)
            else:
                continue

            # scale and draw
            scaled_coverage = self.scale_points(cov_coords, all_points)
            # optional: draw coverage boundary points
            self.canvas.create_polygon(
                scaled_coverage,
                fill="#fef300",
                outline="orange",
